...
void print_bitboard(Bitboard b) {
    for (Rank r = RANK_8; r >= RANK_1; r--) {
        ...
        for (File f = FILE_A; f <= FILE_H; f++) {
            cout << "| " << (bit_is_set(b, make_square(f, r))? 'X' : ' ') << ' ';
        }
        cout << "|" << endl;
    }
    cout << "+---+---+---+---+---+---+---+---+" << endl;
...
namespace {
    ...
    Bitboard sliding_attacks(int sq, Bitboard block, int dirs, int deltas[][2], int fmin=0, int fmax=7, int rmin=0, int rmax=7) {
        ...
        for (int i = 0; i < dirs; i++) {
            ...
            while ((dx == 0) || (f >= fmin && f <= fmax) && (dy == 0) || (r >= rmin && r <= rmax))  {
                result |= (1ULL << (r * 8 + f));
                if (block & (1ULL << (r * 8 + f))) {
                    break;
                }
    ...
    const SquareDelta step[8] = { DELTA_E, DELTA_W, DELTA_N, DELTA_S, DELTA_NE, DELTA_SW, DELTA_NW, DELTA_SE };
